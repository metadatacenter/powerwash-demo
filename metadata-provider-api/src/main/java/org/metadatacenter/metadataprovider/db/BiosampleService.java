...
public class BiosampleService {
  ...
  public ApiOutput search(Map<String, String> attributesAndValuesFilter, boolean includeAccessions,
    ...
    final String TISSUE_PROVIDER_ATTRIBUTE_NAME = "biomaterial provider";
    ...
    Map<String, UniqueBiosampleAttributeValue> tissueProviderValues = new HashMap<>();
    ...
    for (Biosample sample : allSamples) {
      ...
      // Tissue providers
      if (aggregations.contains(BiosampleResource.Aggregation.tissueProvider)) {
        BiosampleAttribute tissueProviderAttribute = sample.extractAttribute(TISSUE_PROVIDER_ATTRIBUTE_NAME);
        addToUniqueAttributeValuesMap(tissueProviderValues, tissueProviderAttribute, isAnnotatedSamplesCollection);
      }
    ...
    AttributeAggregations attAggregations =
        new AttributeAggregations(
            sortBiosampleAttributeValueMap(tissueProviderValues),
            sortBiosampleAttributeValueMap(diseaseValues),
  ...
  private boolean isValidValue(BiosampleAttribute attribute, boolean isAnnotated) {
    // Ignore if the data is annotated or not
    return true;
  }
  ...
  private Map<String, UniqueBiosampleAttributeValue> addToUniqueAttributeValuesMap(
    ...
    if (attribute != null && isValidValue(attribute, annotated)) {
      ...
      // Ignore if the data is annotated or not
      String key = attribute.getAttributeValue().toLowerCase();
